...
export function useJobs(filters?: { status?: string; leaderId?: string }) {
  const queryKey = [api.jobs.list.path, filters?.status, filters?.leaderId].filter(Boolean);
...
export function useCreateJob() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Job Created", description: "Your cleanup job is now live!" });
    },
    onError: (err) => {
...
export function useUpdateJob() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...data }: { id: number } & Partial<InsertJob> & { status?: string, selectedWorkerId?: number }) => {
